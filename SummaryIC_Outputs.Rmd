...
wd <- "C:/users/Nathan/Dropbox/ICES/interCatchSummary-master/"
# output_wd <- "C:/users/Nathan/Dropbox/ICES/interCatchSummary-master/"
...
```{r}
### read the data
test <- scan(paste(wd,"CatchAndSampleDataTablesTimeSeries.txt",sep=""),what='character',sep='\t')
...
##Time-series of catch and discard reporting

Next temporal trends in total catch and discard reporting vs raised discard numbers are inspected 

```{r}
SummaryReport <- table2_bis[,list(CATON=sum(CANUM*WECA)/1000000), by=c('Year','CatchCategory','RaisedOrImported')]

AnnualLandings<-matrix(NA,nrow=3,ncol=length(unique(SummaryReport$Year)))
for(i in 1:length(unique(SummaryReport$Year)))
{
  AnnualLandings[3,i]<-SummaryReport[(SummaryReport$Year==sort(unique(SummaryReport$Year))[i] & SummaryReport$CatchCategory=='Discards' & SummaryReport$RaisedOrImported=='Raised_Discards'),4][[1]]
  
  AnnualLandings[2,i]<-SummaryReport[SummaryReport$Year==sort(unique(SummaryReport$Year))[i] & SummaryReport$CatchCategory=='Discards' & SummaryReport$RaisedOrImported=='Imported_Data',4][[1]]
  
  AnnualLandings[1,i]<-SummaryReport[SummaryReport$Year==sort(unique(SummaryReport$Year))[i] & SummaryReport$CatchCategory=='Landings' & SummaryReport$RaisedOrImported=='Imported_Data',4][[1]]
}
barplot(AnnualLandings,names=sort(unique(SummaryReport$Year)),col=c('blue','yellow','orange'),ylab='CATON',xlab='Year')
legend('topright',legend=c('Landings','Imported Discards','Raised Discards'),fill=c('blue','yellow','orange'),bg='white')


```
`r figcap(name="CatchDiscByYr",caption="Catch represented in landings, imported discards, or raised discards by year")`

Next temporal trends in total catch (Landings+imported discards+raised discards) by gear are inspected 

```{r}

table2_bis <- table2_bis[, Gear:=substr(table2_bis$Fleet,1,3)]
summCatchPerGear <- table2_bis[, list(sumCatch=sum(CANUM*WECA/1000000)), by=c('Year','Gear')]

AnnualCatchGear<-matrix(NA,nrow=length(unique(summCatchPerGear$Gear)),ncol=length(unique(summCatchPerGear$Year)))
for(i in 1:length(unique(summCatchPerGear$Year)))
{
  for(j in 1:length(unique(summCatchPerGear$Gear)))
  {
    AnnualCatchGear[j,i]<-summCatchPerGear[(summCatchPerGear$Year==sort(unique(summCatchPerGear$Year))[i] & summCatchPerGear$Gear==unique(summCatchPerGear$Gear)[j]),3][[1]]
  }
}
possibleCols<-c('red','blue','green','yellow','pink','orange','purple','brown','navy','turquoise','thistle','sienna1','springgreen','steelblue','violet')
colorVals<-possibleCols[1:length(unique(summCatchPerGear$Gear))]
barplot(AnnualCatchGear,names=sort(unique(summCatchPerGear$Year)),col=colorVals,ylab='CATON',xlab='Year')
legend('topright',legend=unique(summCatchPerGear$Gear),fill=colorVals,bg='white')


```
`r figcap(name="CatchByGear",caption="Annual catch by gear")`

Next temporal trends in total catch (Landings+imported discards+raised discards) by area are inspected 

```{r}

summCatchPerArea <- table2_bis[, list(sumCatch=sum(CANUM*WECA/1000000)), by=c('Year','Area')]

AnnualCatchArea<-matrix(NA,nrow=length(unique(summCatchPerArea$Area)),ncol=length(unique(summCatchPerArea$Year)))
for(i in 1:length(unique(summCatchPerArea$Year)))
{
  for(j in 1:length(unique(summCatchPerArea$Area)))
  {
    AnnualCatchArea[j,i]<-summCatchPerArea[(summCatchPerArea$Year==sort(unique(summCatchPerArea$Year))[i] & summCatchPerArea$Area==unique(summCatchPerArea$Area)[j]),3][[1]]
  }
}
possibleCols<-c('red','blue','green','yellow','pink','orange','purple','brown','navy','turquoise','thistle','sienna1','springgreen','steelblue','violet')
colorVals<-possibleCols[1:length(unique(summCatchPerArea$Area))]
barplot(AnnualCatchArea,names=sort(unique(summCatchPerArea$Year)),col=colorVals,ylab='CATON',xlab='Year')
legend('topright',legend=unique(summCatchPerArea$Area),fill=colorVals,bg='white')


```
`r figcap(name="CatchByArea",caption="Annual catch by area")`